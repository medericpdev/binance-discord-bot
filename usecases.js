...
function setBetInConfig(playerName, bet, config) {
  ...
  return { message: `**:white_check_mark: ${PLAYER.name} bet : ${OLD_BET}$ -> ${PLAYER.name} bet : ${bet}$**` };
}
...
async function getPlayerBalanceFutures(apiKey, secretKey) {
  ...
  if (isNaN(parseInt(FUTURES['totalMarginBalance'])))
    return 0;
  else
    return parseInt(FUTURES['totalMarginBalance']);
}
...
function getPnlHistory(message, playerName, pnl, db) {
  ...
  const YESTERDAY_DATE = db.get('data').find({ player_name: playerName, year: YESTERDAY.getFullYear(), month: YESTERDAY.getMonth() + 1, day: YESTERDAY.getDate() }).value();
  const LAST_WEEK_DATA = db.get('data').find({ player_name: playerName, year: LAST_WEEK.getFullYear(), month: LAST_WEEK.getMonth() + 1, day: LAST_WEEK.getDate() }).value();
  const LAST_MONTH_DATA = db.get('data').find({ player_name: playerName, year: LAST_MONTH.getFullYear(), month: LAST_MONTH.getMonth() + 1, day: LAST_MONTH.getDate() }).value();
  ...
  for (let [key, value] of MAP) {
    if (!value)
      message.channel.send("**:small_orange_diamond: " + key + " :** *No data*");
    else {
      balanceNow = pnl - value.pnl;
      message.channel.send("**:small_orange_diamond: " + key + "** *(" + value.day + "/" + value.month + "/" + value.year + ")* : **" + balanceNow + "$**");
    }
  }
}
...
async function saveData(config, db) {
  ...
  for (const PLAYER of PLAYERS) {
    ...
    if (db.get('data').find({ player_name: PLAYER.name, year: YEAR, month: MONTH, day: DAY }).value() == null)
      db.get('data').push({ player_name: PLAYER.name, balance: balanceTotal, year: YEAR, month: MONTH, day: DAY, pnl: PNL }).write()

    console.log("Saving " + PLAYER.name + "'s data (" + DAY + "/" + MONTH + "/" + YEAR + " - " + HOUR + ":" + MINUTE + ")");
  }
}
